...
#   *** diffractionPatron pour pas de temps plus petits ***
...
def diffractionPatron(mod_psis, L, Ny, s, a, k, D, n0=0, extract_frac=0.75, x0=None, Dt=None, v_g=None):
    ...
    # Trouver n_stop
    max_intensity_at_extract = np.max([np.sum(np.abs(psi[:, j_extract])**2) for psi in mod_psis])
    threshold = 0.03 * max_intensity_at_extract
    n_stop = len(mod_psis)
    for n, psi in enumerate(mod_psis, start=n0):
        intensity_at_extract = np.sum(np.abs(psi[:, j_extract])**2)
        if intensity_at_extract > threshold:
            n_stop = min(len(mod_psis), n - n0 + 10)
            break

    print(f"n_stop détecté à n={n_stop + n0}, t={(n_stop + n0)*Dt:.4f} s")

    # Cumul de l'intensité
    cumulative_intensity = np.zeros(mod_psis[0].shape[0])
    for psi in mod_psis[:n_stop]:
        cumulative_intensity += np.abs(psi[:, j_extract])**2

    # Diagnostic : intensité à x_extract
    intensities = [np.sum(np.abs(psi[:, j_extract])**2) for psi in mod_psis]
    times = np.arange(n0, n0 + len(mod_psis)) * Dt
    plt.figure(figsize=(10, 6))
    plt.plot(times, intensities, label='Intensité à x_extract')
    plt.axhline(y=threshold, color='r', linestyle='--', label=f'Seuil = {threshold:.2e}')
    plt.axvline(x=(n_stop + n0) * Dt, color='g', linestyle='--', label=f'n_stop = {n_stop + n0}')
    plt.xlabel('Temps (s)')
    plt.ylabel('Intensité (|ψ|²)')
    plt.title("Évolution de l'intensité à x_extract")
    plt.grid(True)
    ...
    # Lissage et calcul de la largeur des fentes
    window_size = 11
    if window_size >= len(cumulative_intensity):
        window_size = len(cumulative_intensity) // 2 * 2 + 1
    smoothed_intensity = savgol_filter(cumulative_intensity, window_size, 3)
    
    center_idx = len(y_screen) // 2
    minima_indices = argrelextrema(smoothed_intensity, np.less)[0]
    left_minima = minima_indices[minima_indices < center_idx]
    right_minima = minima_indices[minima_indices > center_idx]
    
    if len(left_minima) > 0 and len(right_minima) > 0:
        y_left = y_screen[left_minima[-1]]
        y_right = y_screen[right_minima[0]]
        delta_y = y_right - y_left
        lambda_ = 2 * np.pi / k
        a_calculated = (2 * lambda_ * D) / delta_y
        print(f"Largeur des fentes calculée : {a_calculated:.4f}")
        print(f"Largeur des fentes théorique : {a:.4f}")
    else:
        print("Impossible de détecter les minimas.")
