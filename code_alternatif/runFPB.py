...
if __name__ == "__main__":
    ...
    fact_ar = np.array([0.056, 0.055, 0.054, 0.053, 0.052, 0.051, 0.05, 0.049, 0.048, 0.047], dtype=np.double); # Matrice pleine
    mem_ar=np.zeros(fact_ar.size,dtype=np.double)
    norm_ar=np.zeros(fact_ar.size,dtype=np.double)
    ...
    for fact in fact_ar:
        ...
        # On calcul l'erreur de convergence ici
        norme = np.linalg.norm(mod_psis[int(len(mod_psis) * ((L-3)/L))], ord=2)
        norm_ar[ci] = norme
    ...
    erreur = []
    for err in range(len(norm_ar)):
        erreur.append(np.abs((norm_ar[-1]-norm_ar[err])/norm_ar[err]))
    plt.plot(d_ar, erreur, label='Erreur de convergence')
    plt.xlabel("Pas d'espace")
    plt.ylabel("Erreur de convergence")
    plt.title("Évolution de l'erreur de convergence en fonction du pas d'espace")
    plt.legend()
    plt.grid(True)
    plt.show()
