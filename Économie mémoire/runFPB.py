...
if __name__ == "__main__":
    ...
    fact_ar = np.array([0.05], dtype=np.double)
    ...
    for fact in fact_ar:
        ...
        D = abs(x_extract - distance_to_fentes)
        
        tracemalloc.start()
        solvemat_t = time()
        mod_psis, initial_norm, norms = solveMatrix(A, M, L, Nx, Ny, Ni, Nt, x0, y0, Dy, k, sigma, n0) # mod_psis est une liste de matrices (𝑁x−2)x(Ny-2)
    ...
    final_psi = diffractionPatron(mod_psis, L, Ny, s, a, k, D, extract_frac, x0, Dt, v_g)
