...
def convergence(mod_psis_ar, d_ar):
    ...
    plt.plot(log_d, log_err, 'o', label='Erreur d\'ordre ~2 de la simulation')
    plt.plot(log_d, fit, '--', label=f"Régression linéaire")
    plt.xlabel("log(Pas d'espace)")
    plt.ylabel("log(Erreur de convergence)")
    plt.title("Logarithme d'erreur de convergence numérique d'ordre 2 en fonction du pas d'espace")
    plt.legend()
    plt.grid(True)
    plt.show()
