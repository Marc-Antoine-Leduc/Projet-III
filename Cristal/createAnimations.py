...
def makeAnimationForPeriodicSlits(mod_psis, v, L, Nt, n0, v_g, Dt, x0, j0, j1, w, Dy, extract_frac, x_fentes, x_extract, D, sigma):
    import matplotlib.pyplot as plt
    from matplotlib.animation import FuncAnimation
    from matplotlib.patches import Rectangle
    import os
    ...
    # --- Affichage des murs du potentiel périodique ---
    slitcolor = "gray"
    slitalpha = 0.8

    for i in range(v.shape[0]):
        if np.any(v[i, j0:j1] != 0):
            y_start = i * Dy
            y_end = (i + 1) * Dy
            wall = Rectangle((j0 * Dy, y_start), w, Dy,
    ...
    ax.axvline(x=x_T, color='purple', linestyle=':', linewidth=2, label=f'Limite T (x={x_T:.2f})')
    ax.axvline(x=x_extract, color='cyan', linestyle='--', linewidth=2, label='Patron extrait')
    ax.axvline(x=x_n0, color='green', linestyle='-.', linewidth=2, label=f'Début cumul (n0={n0}, x={x_n0:.2f})')
    ax.hlines(y=0, xmin=x_fentes, xmax=x_extract, colors='yellow', linestyles='-', linewidth=5, label=f'Distance D = {D:.2f}', zorder=100)
    ax.text((x_fentes + x_extract) / 2, 0.5, f'D = {D:.2f}', color='yellow', fontsize=10, ha='center', va='bottom', zorder=101)
    ...
    output_dir = "."
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(os.getcwd(), "basicAnimation.mp4")
    # output_file = os.path.join(output_dir, "basicAnimation.mp4")
    print(f"Enregistrement de l'animation dans : {output_file}")
    anim.save(output_file, writer="ffmpeg", fps=60)
...
def makeAnimationForGaussianSlits(mod_psis, v, L, Nt, n0, v_g, Dt, x0, j0, j1, w, Dy, extract_frac, x_fentes, x_extract, D, sigma, s):
    import matplotlib.pyplot as plt
    from matplotlib.animation import FuncAnimation
    from matplotlib.patches import Rectangle
    import os
    ...
    # Position du mur en x
    x_wall = j0 * Dy

    # Tracé du mur complet
    wall_rect = Rectangle((x_wall, 0), w, L, color=slitcolor, alpha=slitalpha, zorder=5)
    ax.add_patch(wall_rect)

    # --- Ajout des fentes gaussiennes comme "trous" visuels ---
    y_vals = np.linspace(0, L, v.shape[0])
    slit_centers = np.arange(s / 2, L, s)
    for yc in slit_centers:
        # On dessine des rectangles transparents à la position des fentes pour les rendre visibles
        # (ceci est une simplification visuelle : on ne modifie pas réellement V ici)
        hole_height = 4 * sigma  # large en fonction de sigma
        hole_bottom = max(0, yc - hole_height / 2)
        hole_top = min(L, yc + hole_height / 2)
        hole = Rectangle((x_wall, hole_bottom), w, hole_top - hole_bottom,
                         color="white", alpha=1.0, zorder=6)
        ax.add_patch(hole)

    # --- Lignes verticales de repères ---
    x_T = x0 + v_g * (Nt * Dt)
...
def diffractionPatronGauss(mod_psis, L, Ny, s, a, k, D, n0=0, extract_frac=0.75, x0=None, Dt=None, v_g=None):
    """
    Affiche le patron de diffraction cumulé sur l'écran à x = extract_frac * L,
    en ne prenant en compte que les instants après n0, jusqu'à ce que x_extract soit atteint.
    Calcule et affiche la vitesse de propagation expérimentale.
    ...
    if len(left_minima) > 0 and len(right_minima) > 0:
        ...
        print(f"Largeur des fentes calculée : {a_calculated:.4f}")
        print(f"Largeur des fentes théorique : {s:.4f}")
    else:
        y_left, y_right = None, None
        a_calculated = None
        print("Impossible de détecter les minimas.")
